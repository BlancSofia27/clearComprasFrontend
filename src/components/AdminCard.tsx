...
const MyAdminCard: React.FC<MyAdminCard> = ({ post }) => {
  ...
  return (
    ...
    <div className="relative">
      ...
      {/* Botones Editar y Eliminar superpuestos sobre la imagen */}
      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-80 opacity-0 hover:opacity-100 transition-opacity duration-300">
        <button
    ...
    <div className="xl:p-4 xs:p-2 xl:text-md xs:text-sm xs:mt-3">
      ...
      <div className="mb-2">
        ...
        <div className="flex flex-row gap-2 overflow-x-auto p-2 h-16">
          {size.map((s, index) => (
            <span
              key={index}
              className="bg-gray-200 text-gray-700 px-2 py-1 rounded h-7"
            >
              {s}
            </span>
          ))}
      ...
      >
        <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full overflow-y-auto xs:max-h-[80vh] xl:max-h-[90vh]">
